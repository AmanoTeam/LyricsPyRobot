...
async def teor(c, m):
    ...
    if m.from_user.id == int(user) or m.from_user.id in sudos:
        ...
        if not n:
            ...
            await m.edit_message_text('{} - {}\n{}'.format(a["musica"], a["autor"], n[1]).encode("windows-1252", "backslashreplace").decode("unicode_escape"), reply_markup=keyboard)
    else:
...
async def tetr(c, m):
    ...
    if m.from_user.id == int(user) or m.from_user.id in sudos:
        ...
        if not n:
            ...
            await m.edit_message_text('{} - {}\n{}'.format(a["musica"], a["autor"], n[2]).encode("windows-1252", "backslashreplace").decode("unicode_escape"), reply_markup=keyboard)
    else:
        a = await c.get_chat(int(user))
        await m.answer(f'Você n pode mecher nisso, somente o {a.first_name} {a.last_name} pode')
...
async def tr(c, m):
    ...
    if m.from_user.id == int(user) or m.from_user.id in sudos:
        ...
        if not n:
            ...
            await m.edit_message_text('[{} - {}]({})\n{}'.format(a["musica"], a["autor"], a['link'], a['traducao'])[:4096].encode("windows-1252", "backslashreplace").decode("unicode_escape"), reply_markup=keyboard, disable_web_page_preview=True)
    else:
        a = await c.get_chat(int(user))
        await m.answer(f'Você n pode mecher nisso, somente o {a.first_name} {a.last_name} pode')
